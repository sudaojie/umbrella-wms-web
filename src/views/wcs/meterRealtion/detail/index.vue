  ...
  <div class="app-container" style="background-color: #f0f2f5;">
    ...
    <div style="background-color: white;margin-top: 10px;padding: 10px;border-radius: 10px;">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="电表设备" prop="meterDeviceNo">
          <el-select v-model="form.meterDeviceNo" filterable placeholder="请选择电表设备" clearable value="">
            <el-option
        ...
        <el-divider content-position="center">设备详情信息</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="openSelectGoods" v-hasPermi="['wcs:gateWayDetailRealtion:add']">添加设备</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" icon="el-icon-delete" size="mini"
                       @click="handleDeleteInbillDetail" v-hasPermi="['wcs:gateWayDetailRealtion:remove']">删除
            </el-button>
          </el-col>
        </el-row>
      ...
      <div slot="footer" style="text-align: center;margin-top: 10px;">
        <el-button icon="el-icon-save" type="primary" @click="submitForm" v-hasPermi="['wcs:gateWayDetailRealtion:save']">保 存</el-button>
        <el-button icon="el-icon-close" @click="cancel">关 闭</el-button>
      </div>
    </div>
    <!-- 设备数据-选择对话框 -->
    <el-dialog title="设备选择" :visible.sync="selectGoods.open" width="50%" append-to-body>
...
<script>
  ...
  import { listTypeDeviceInfos, addMeterRealtion, queryRelationById } from "@/api/wcs/meterRealtion";
  import { getMeterDeviceInfoList } from "@/api/wcs/deviceBaseInfo";
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          // meterDeviceNo: [
          //     { required: true, message: "电表设备不能为空", trigger: "blur" }
          // ]
        },
        deviceInfoList: [],
        deviceType: '6',
    ...
    methods: {
      ...
      getSelectGoodsList() {
        ...
        let result = []
        getMeterDeviceInfoList(this.selectDevice.queryParams).then(res => {
      ...
      submitForm() {
        this.$refs['form'].validate(valid => {
          if (valid) {
            if (!this.form.meterDeviceNo) {
              this.$modal.msgError('请维护电表设备')
              return
            }
            if (this.inbillDetailLists.length == 0) {
              this.$modal.msgError('设备详情信息不可为空')
              return
            }
            let arr  = this.inbillDetailLists.map(item => item.deviceNo)
            if (arr) {
              this.form.deviceNo = arr.join(",")
            } else {
              this.form.deviceNo = ''
            }
            ...
            if (this.form.id != null) {
              ...
              addMeterRealtion(this.form).then(response => {
