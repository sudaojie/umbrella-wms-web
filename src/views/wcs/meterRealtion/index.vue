  ...
  <div class="app-container">
      ...
      margin-bottom: 15px;background: #f9f9f9;border-radius: 10px 10px 0px 0px;color: #00000091;" >
        ...
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="125px" style="padding-left: 1px;">
          <el-form-item label="电表设备" prop="meterDeviceNo">
            <el-select v-model="queryParams.meterDeviceNo" filterable placeholder="请选择电表设备" clearable value="" @change="handleQuery">
              <el-option
    ...
    <div style="background-color: white;margin-top: 10px;padding: 10px;border-radius: 10px;">
        ...
        <el-table v-loading="loading" :data="realtionList" :height="tableHeight" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="电表设备名称" align="center" prop="initDeviceName" show-overflow-tooltip/>
          <el-table-column label="堆垛机数量" align="center" prop="stackerNum" show-overflow-tooltip/>
    ...
    <el-dialog :title="title" :visible.sync="viewModalOpen" append-to-body>
        <el-descriptions :column="2" border>
          <el-descriptions-item label="电表设备编号">{{ viewForm.meterDeviceNo }}</el-descriptions-item>
...
<script>
import { listTypeDeviceInfos, getRealtion, addRealtion, updateRealtion, listRealtion } from "@/api/wcs/meterRealtion";
import { delRealtion } from "@/api/wcs/gateWayRealtion";
import { getToken } from "@/utils/auth";
export default {
  name: "wcsMeterRealtion",
  data() {
    return {
      addDeviceInfoList: [],
      // 遮罩层
      ...
      rules: {
        meterDeviceNo: [
            { required: true, message: "电表设备编号不能为空", trigger: "blur" }
        ],
        noIpDeviceNo: [
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.meterDeviceNo)
      this.single = selection.length!==1
      this.multiple = !selection.length
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.$tab.openPage("新增电表-设备关系", "/wcs/wcsMeterRealtion/detail");
    },
    /** 详情按钮操作 */
    handleView(row){
      const id = row.id;
    ...
    handleUpdate(row) {
      ...
      this.$tab.openPage("修改电表-设备关系", "/wcs/wcsMeterRealtion/detail",{id:row.meterDeviceNo});
    },
    /** 提交按钮 */
    submitForm() {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const ids = row.meterDeviceNo || this.ids;
