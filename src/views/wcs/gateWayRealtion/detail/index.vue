  ...
  <div class="app-container" style="background-color: #f0f2f5;">
    ...
    <el-dialog title="传感器选择" :visible.sync="selectGoods.open" width="50%" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="selectGoods.open = false, selectDevice.queryParams.deviceNo = null">取 消</el-button>
      </div>
    </el-dialog>
...
<script>
  ...
  import { listTypeDeviceInfos, addRealtion, addGateWayRealtion } from '@/api/wcs/freshAirThtbRealtion'
  import { queryRelationById } from "@/api/wcs/gateWayRealtion";
  import { getSensorInfoList, getGateWayDeviceInfoList } from "@/api/wcs/deviceBaseInfo";
  ...
  export default {
    ...
    methods: {
      ...
      getSelectGoodsList() {
        ...
        let result = []
        getGateWayDeviceInfoList(this.selectDevice.queryParams).then(res => {
      ...
      submitForm() {
        ...
        this.$refs['form'].validate(valid => {
          if (valid) {
            if (!this.form.gateWayDeviceNo) {
              this.$modal.msgError('请维护网关设备')
              return
            }
            ...
            if (arr) {
              this.form.noIpDeviceNo = arr.join(",")
            } else {
              this.form.noIpDeviceNo = ''
            }
            ...
            if (this.form.id != null) {
              ...
              addGateWayRealtion(this.form).then(response => {
                // this.form = response.data
